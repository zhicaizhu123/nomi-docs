...
它会生产一个`xxx.map`文件，里面包含源代码和构建后代码的每一行，每一列的映射关系，当构建后代码出错了，会通过`xxx.map`文件从构建后的代码出错位置找到映射后源代码出错位置，从而让浏览器提示源代码文件出错的位置，帮助我们更快的定位到问题。
...
### 解决js兼容性问题 core-js
我们使用 `babel` 对 js 进行兼容性处理，其实使用` @babel/preset-env` 智能预设来处理兼容性问题。

它将ES6的一些语法进行编译转化，比如箭头函数，但是如果是ES6+以上的语法，例如`async/await`,、`Promise`、 `includes`等，它是没有办法处理的。
