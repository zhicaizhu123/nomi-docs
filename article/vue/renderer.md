...
## 各个模块有机结合
综上所述，**组件的实现依赖渲染器，模板的编译依赖编译器，并且编译后生成的代码是根据渲染器和虚拟DOM的设计决定的**，所以Vue的各个模块是相互关联，相互制约的，共同形成一个有机整体。

假设有以下模板
```html
<div id="foo" :class="cls"></div>
```
根据上面的知识，我们很清楚编译器会把它编译成一个渲染函数
```javascript
render() {
    // return h('div', { onClick: onClick }, 'click me')
    // 为了方便理解，我们返回一个虚拟DOM对象
    return {
        tag: 'div',
        props: {
            id: 'foo',
...
```
我们可以发现，id属性是一个常量，而 `cls` 是一个变量，它随时会发生变化。**编译器可以分析动态内容，并在编译阶段把这些信息提取出来，然后直接给到渲染器，这样渲染器就不需要花费大力气来寻找变更点了，所以编译器能够识别出哪些是静态属性，哪些事静态属性，在生成代码的时候完全可以附带这些信息。**
...
render() {
    ...
    return {
        ...
        patchFlag: 1 // 假设1代表class是动态的
    }
}
```
上面代码多出一个 `patchFlag` 属性，渲染器看到这个标记就知道 `class` 是一个动态变化的，相当于省去了查找变更点的工作量，性能自然就提升了。

综上，我们了解到**编译器和渲染器之间是存在信息交流的，它们互相配合使得性能进一步提升，它们之间交流的媒介就是虚拟DOM对象。**